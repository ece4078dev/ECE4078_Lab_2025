...
if __name__ == '__main__':
    ...
    print("Rotation Angle (Degrees): {}".format(np.degrees(theta)))
    ...
    # Render image showing the ground truth and aligned predicted locations
    # Show the aruco marker number
    # Print error distances
    for index, tag in sorted(enumerate(taglist), key=lambda x: x[1]):
        error = np.linalg.norm(us_vec_aligned[:, index] - gt_vec[:, index])
        print("Marker {}: Error = {:.4f}m".format(tag, error))
        plt.annotate(tag, (us_vec_aligned[0, index], us_vec_aligned[1, index]), textcoords="offset points", xytext=(0,10), ha='center', color='red')
        plt.annotate(tag, (gt_vec[0, index], gt_vec[1, index]), textcoords="offset points", xytext=(0,10), ha='center', color='green')

    # Add rmse to 4 decimal places
    plt.title("Ground Truth vs Aligned Predicted Locations, RMSE: {:.4f}".format(rmse))
    ...
    plt.scatter(us_vec_aligned[0, :], us_vec_aligned[1, :], color='red', label='Aligned Predicted Locations')
    plt.scatter(gt_vec[0, :], gt_vec[1, :], color='green', label='Ground Truth Locations')
    plt.legend()
    print("The RMSE after alignment: {}".format(rmse))
    plt.show()
